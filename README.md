...
Stages 0–6.5 established and validated the versioned Manifest, pure policy, GitHub adapters, Actions-first operation chain, Relay, lifecycle CLI, release contracts, and Sandbox/blank-repository/mature-repository migration evidence. Stage 6.6 is now migrating that proven behavior to an App-first central event-driven runtime. `packages/control` contains the runtime-neutral Classification and DCO Advisory kernel used by the existing Action adapter. The central foundation is split into a bounded HMAC Ingress, an at-least-once Queue, independent per-PR and per-repository SQLite Durable Object Coordinators, and a private versioned Control adapter. Repository fan-out source accepts a strict `ScopeWorkItem V1`, reads one bounded live GitHub App page per invocation, requires two identical complete passes, and emits one deterministic `WorkItem V3` per open pull request. The repository object persists page receipts, target state, Queue confirmations, and deterministic replay state so an ambiguous response can safely redeliver the same child identities to the per-PR deduplication boundary. This repository fan-out slice is source-integrated only: its new Durable Object export has not been deployed and the GitHub App repository event has not been enabled. A separate least-authority diagnostics gateway remains the only general public read-only evidence surface: Cloudflare Access authenticates it, independent read-only Cloudflare tokens observe deployment and Queue state, and private Control rebinds every request to the live GitHub App installation and repository identity. An isolated Access-protected Recovery plane is being source-integrated as the bounded public operator command surface: it exposes only its recovery ledger inspection and explicit mutations, durably captures the event DLQ, replays selected canonical entries through the main Queue, and asks private Control to enumerate or redeliver failed GitHub App deliveries without receiving GitHub or Cloudflare account credentials. Recovery and its new SQLite Durable Object are not deployed. Real pull-request operations remain fail-closed, the GitHub App webhook is not switched, and the legacy Relay deploy is manual-only until delivery recovery and later Control slices are proven. The remaining reusable workflows, thin callers, and bundled Action surfaces are transitional execution and regression evidence. Stage 7 consumer migration has not started.
...
- `packages/coordinator/`: private Queue consumer, fresh-wakeup producer, and independent per-PR and per-repository SQLite Durable Object host.
