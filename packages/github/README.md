...
`GitHubRepositoryClient` is the shared integration adapter for the default-branch Manifest loader, bounded PR/Check/workflow/review-thread reads, and one-call/one-mutation primitives. GitHub.com callers can use one transport; GitHub Enterprise Server callers pass separately confined REST (`https://HOSTNAME/api/v3/`) and GraphQL (`https://HOSTNAME/api/`) transports so `/graphql` resolves to the platform's distinct GraphQL endpoint. The client does not decide policy, retry in time, poll for convergence, or choose which mutations to execute; Action operations use core plans to make those choices.
