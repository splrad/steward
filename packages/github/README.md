...
`GitHubRepositoryClient` is the shared integration adapter for the default-branch Manifest loader, bounded PR/Check/workflow/review-thread reads, and one-call/one-mutation primitives. It does not decide policy, retry in time, poll for convergence, or choose which mutations to execute; Action operations use core plans to make those choices.
