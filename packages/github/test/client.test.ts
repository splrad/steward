...
describe("代码托管平台客户端", () => {
  ...
  it("只允许五个中央工作流并从中央仓库实时默认分支派发", async () => {
    const requests: unknown[] = [];
    const client = {
      getRepository: async () => ({ default_branch: "trunk" }),
      request: async (...args: unknown[]) => { requests.push(args); },
    } as unknown as GitHubClient;
    for (const workflow of ["onboard-repository.yml", "pr-automation.yml", "pr-classification.yml", "pr-validation-gate.yml", "release.yml"]) {
    ...
    expect(requests[3]).toEqual(["POST", "/repos/splrad/steward/actions/workflows/pr-validation-gate.yml/dispatches", { ref: "trunk", inputs: { value: "x", policySha: "a".repeat(40) } }]);
    expect(requests[4]).toEqual(["POST", "/repos/splrad/steward/actions/workflows/release.yml/dispatches", { ref: "trunk", inputs: { value: "x" } }]);
