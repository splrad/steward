...
type RelayPolicy = 'enabled' | 'disabled' | 'missing' | 'invalid';
...
async function loadRelayPolicy(
  ...
  const url = new URL(`https://api.github.com/repos/${repository}/contents/${MANIFEST_PATH}`);
  url.searchParams.set('ref', defaultBranch);
  const manifestResponse = await dependencies.fetch(url, { headers: githubHeaders(token) });
  if (manifestResponse.status === 404) return 'missing';
  if (!manifestResponse.ok) throw new Error(`Manifest lookup failed (${manifestResponse.status})`);

  let file: RepositoryFilePayload;
  try {
    file = await manifestResponse.json() as RepositoryFilePayload;
  } catch {
    return 'invalid';
  }
  if (file.type !== 'file' || file.encoding !== 'base64' || !file.content) return 'invalid';

  try {
    const manifest = parseManifest(JSON.parse(decodeBase64(file.content)));
    return manifest.features.webhookRelay ? 'enabled' : 'disabled';
  } catch {
    return 'invalid';
  }
}
...
export async function handleRequest(
  ...
  try {
    policy = await loadRelayPolicy(dependencies, token, repository, defaultBranch);
  } catch {
    await releaseDelivery(coordinator);
    return response(502, 'GitHub manifest lookup failed');
  }
  if (policy !== 'enabled') {
    await releaseDelivery(coordinator);
    if (policy === 'missing') return response(202, 'Ignored: Steward manifest not found');
    if (policy === 'disabled') return response(202, 'Ignored: Steward relay disabled');
    return response(422, 'Invalid Steward manifest');
  }
