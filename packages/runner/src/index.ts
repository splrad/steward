...
const safeGeneratedSummaryValidationMessage = /^(?:Copilot结果必须是对象|Copilot结果包含额外字段|classification字段缺失|type无效|scope无效|title格式无效|changes无效|related无效|(?:title|summary|motivation|changes\[\]|impact|impact\[\]|related\[\]|releaseAndMigration|releaseAndMigration\[\])(?:必须是字符串|长度或格式无效|无效))$/u;
...
async function reconcileManagedFile(input: { repository: any; gh: GitHubClient; token: string; path: string; content: string; branch: string; title: string; policySha: string; deliveryId: string; redrive: DurableBodyRedrive }): Promise<"unchanged" | "pull-request-created" | "pull-request-updated"> {
...
async function onboard(args: Readonly<Record<string, string>>) {
  ...
  const { token, client: gh } = await clientWithToken(repositoryId, { administration: "write", contents: "write", pull_requests: "write", checks: "read", metadata: "read", members: "read" }, policySha);
  ...
  const result = await reconcileManagedFile({ repository, gh, token, path: instructions.targetPath, content: instructions.content, branch: planned.branch, title: planned.title, policySha, deliveryId,
...
async function syncInstructions(args: Readonly<Record<string, string>>) {
  ...
  const results = await runManagedRepositorySync(targets, async ({ repository, configuration }) => {
    ...
    const { token, client: gh } = await clientWithToken(repository.id, { contents: "write", pull_requests: "write", checks: "read", metadata: "read" }, policySha);
