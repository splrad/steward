...
export async function planInitFiles(input: {
  ...
  if (profile) {
    ...
    const generatedRemovals = [...profile.remove.keys()].filter((filePath) => generatedByPath.has(filePath));
    if (generatedRemovals.length) {
      throw new Error(`adoption profile removal paths are generated by this init spec: ${generatedRemovals.join(', ')}`);
    }
  }
