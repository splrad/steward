...
export interface OnboardingConfiguration { managed: boolean; reviewInstructionsProfile?: string; classification?: { profile: string }; validationProfile?: string; releaseProfile?: string | null }
...
export function validateRepositoryForOnboarding(repository: RepositoryForOnboarding, organizationId: number, configuration: OnboardingConfiguration): 'ready' | 'waiting-for-default-branch' {
  ...
  if (repository.visibility === 'private' && (!configuration.classification?.profile || !configuration.validationProfile || !configuration.reviewInstructionsProfile)) throw new Error('私有仓库必须先配置专用覆盖');
...
export function renderOnboardingPullRequest(input: { template: string; configuration: Required<Pick<OnboardingConfiguration, 'reviewInstructionsProfile' | 'classification' | 'validationProfile'>> & Pick<OnboardingConfiguration, 'releaseProfile'>; actor: string; context: string }): { title: string; body: string; branch: string } {
  const generated: GeneratedSummary = {
    ...
    changes: [
      ...
      `生成${input.configuration.reviewInstructionsProfile}代码审查规则集，并设置压缩合并只使用拉取请求标题且不生成扩展描述，同时清理来源分支`,
...
export function verifyOnboardingReadback(actual: Partial<RepositorySettings>, expectedInstructions: Readonly<Record<string, string>>, actualInstructions: Readonly<Record<string, string>>, workflowPaths: readonly string[], allowedWorkflowPaths: readonly string[]): boolean {
  ...
  if (Object.keys(expectedInstructions).sort().join('\n') !== Object.keys(actualInstructions).sort().join('\n')) return false;
  for (const [path, content] of Object.entries(expectedInstructions)) if (actualInstructions[path] !== content) return false;
