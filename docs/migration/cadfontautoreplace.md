...
The reviewed source snapshot remains encoded as the historical `cadfontautoreplace-f6331185` adoption fixture. It records seven exact-digest same-path replacements and twenty-five exact-digest removals for the legacy governance, Matrix, DCO, single-repository Relay, and Relay deployment surfaces, while preserving `.github/scripts/generate-release-notes.ps1`, `.github/pr-classification-rules.json`, and `.github/release.yml` as project-owned Release inputs. The fixture is retained only as migration evidence: public init rejects `adoption`, and the CLI distribution does not include this profile.
