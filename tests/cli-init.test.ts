...
describe('init --dry-run', () => {
  ...
  it('rejects an adoption profile that removes a path generated by the current spec', async () => {
    const directory = await target();
    const configured = manifest();
    configured.features = {
      prAutomation: false, classification: false, dcoAdvisory: false, governance: false,
    ...
    const spec = parseInitSpec({
      stewardSha,
      manifest: configured,
      adoption: { profile: 'test-generated-removal' },
    });
    await expect(createInitPlan({
      spec,
      targetDirectory: directory,
      templateDirectory: adoptionTemplateDirectory,
    })).rejects.toThrow('removal paths are generated by this init spec: .github/dependabot.yml');
  });
